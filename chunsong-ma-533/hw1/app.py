...
app.layout = html.Div([
    ...
    html.Button('Plot', id = 'run-query-1', n_clicks = 0),
...
def render_ab_plot(n_clicks, returns, benchmark_id, asset_id, start_date, end_date):
    ...
    returns1['Date'] = pd.to_datetime(returns1['Date']).dt.date
    returns1 = returns1[(returns1['Date'] >= pd.Timestamp(start_date)) & (returns1['Date'] <= pd.Timestamp(end_date))]
    returns = returns1.to_dict()
    ...
    return plt, f"Alpha: {alpha} | Beta: {beta}"
